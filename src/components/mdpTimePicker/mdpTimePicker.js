...
function TimePickerCtrl($scope, $mdDialog, time, autoSwitch, ampm, $mdMedia) {
    ...
    this.hoursFormat = self.ampm ? 'h' : 'HH';
...
function ClockCtrl($scope) {
    ...
    this.$onInit = function () {
        self.STEP_DEG = 360 / (self.ampm ? 12 : 24);
        self.steps = [];
        this.CLOCK_TYPES = {
            "hours": {
    ...
    this.getPointerStyle = function () {
        ...
        const degrees = Math.round(self.selected * (360 / divider)) - 180;
        return {
...
module.directive('mdpClock', ['$animate', '$timeout', function ($animate, $timeout) {
    return {
        ...
        template: '<md-card class="mdp-clock">' +
            ...
            '</md-toolbar>' +
            '<md-button ng-class="{\'odd\': ((step % 2 > 0) && (timepicker.currentView == timepicker.VIEW_HOURS)), \'md-primary\': ((clock.selected == step) || (step == clock.steps[clock.steps.length - 1] && clock.selected == 0)), \'md-raised\': (raised || (clock.selected == step)) }" ng-mouseenter="raised = true" ng-mouseleave="raised = false" class="md-icon-button mdp-clock-deg{{ ::(clock.STEP_DEG * ($index + 1) * ((timepicker.currentView == timepicker.VIEW_MINUTES && !clock.ampm) ? 2 : 1)) }}" ng-repeat="step in clock.steps" ng-click="clock.setTime(step)">{{ step < 24 || timepicker.currentView == timepicker.VIEW_MINUTES ? step : 0 }}</md-button>' +
            '</div>' +
            '</md-card>',
        controller: ['$scope', ClockCtrl],
        controllerAs: 'clock',
        link: function (scope, element, attrs, ctrl) {
            const pointer = angular.element(element[0].querySelector('.mdp-pointer'));
            const timepickerCtrl = scope.$parent.timepicker;

            scope.raised = false;

            var onEvent = function (event) {
                const containerCoords = event.currentTarget.getClientRects()[0];
                const x = ((event.currentTarget.offsetWidth / 2) - (event.pageX - containerCoords.left));
                const y = ((event.pageY - containerCoords.top) - (event.currentTarget.offsetHeight / 2));
...
module.provider('$mdpTimePicker', function () {
    ...
    this.$get = ['$mdDialog', '$mdpLocale', function ($mdDialog, $mdpLocale) {
        const timePicker = function (time, options) {
            if (!angular.isDate(time)) time = Date.now();
            if (!angular.isObject(options)) options = {};

            const labelOk = options.okLabel || $mdpLocale.time.okLabel || LABEL_OK;
            const labelCancel = options.cancelLabel || $mdpLocale.time.cancelLabel || LABEL_CANCEL;
...
function compareTimeValidator(value, format, otherTime, comparator) {
    ...
    const date = angular.isDate(value) ? moment(value) : moment(value, format, true);
...
module.directive('mdpTimePicker', ['$mdpTimePicker', '$timeout', '$mdpLocale', function ($mdpTimePicker, $timeout, $mdpLocale) {
    return {
        ...
        template: function (element, attrs) {
            const noFloat = angular.isDefined(attrs.mdpNoFloat) || $mdpLocale.time.noFloat;
            const openOnClick = angular.isDefined(attrs.mdpOpenOnClick) || $mdpLocale.time.openOnClick;
        ...
        link: function (scope, element, attrs, controllers, $transclude) {
            ...
            const messages = angular.element(inputContainer[0].querySelector('[ng-messages]'));
            ...
            ngModel.$formatters.unshift(function (value) {
                const time = angular.isDate(value) && moment(value);
                if (time && time.isValid()) {
                    const strVal = time.format(scope.timeFormat);
            ...
            function updateTime(time) {
                const value = moment(time, angular.isDate(time) ? null : scope.timeFormat, true);
                const strValue = value.format(scope.timeFormat);
...
module.directive('mdpTimePicker', ['$mdpTimePicker', '$timeout', function ($mdpTimePicker) {
    return {
        restrict: 'A',
        require: 'ngModel',
        scope: {
